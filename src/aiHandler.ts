...
if (API_KEY && API_KEY.length > 5) {
    console.log(`[DEBUG API KEY] Clave cargada correctamente. Fragmento: ${API_KEY.substring(0, 8)}...`);
} else {
...
export async function generateMeetingSummary(history: Array<{ user: string; text: string }>): Promise<string> {
    ...
    console.log("[DEBUG AI] Texto final de la conversación enviado a Gemini:\n", transcriptText);

    // Verificación de Contenido Mínimo (Evita llamadas si el chat está casi vacío)
    if (transcriptText.trim().length < 20) {
        console.warn("[AI Handler] Input de conversación muy corto. Omitiendo llamada a Gemini.");
    ...
    try {
        ...
        console.log('[DEBUG AI] Respuesta completa del modelo:', JSON.stringify(response, null, 2));
        ...
        if (text.trim() === '') {
            console.error('GEMINI DEVOLVIÓ UN TEXTO COMPLETAMENTE VACÍO. La clave puede estar fallando o la cuota agotada.');
